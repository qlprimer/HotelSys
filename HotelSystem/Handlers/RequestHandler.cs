...
namespace SharpBrowser {
	internal class RequestHandler : IRequestHandler {
		...
		public RequestHandler(CefMainWindow form) {
			myForm = form;
		}
		...
		public bool OnResourceResponse(IWebBrowser browserControl, IBrowser browser, IFrame frame, IRequest request, IResponse response) {
			...
			if (code == 404) {
				...
				if (!request.Url.IsURLLocalhost()) {
					...
					request.Url = CefMainWindow.FileNotFoundURL + "?path=" + request.Url.EncodeURL();
				}
			...
			if (code == 0 && request.Url.IsURLOfflineFile()) {
				...
				if (path.FileNotExists()) {
					...
					// show offline "file not found" page
					request.Url = CefMainWindow.FileNotFoundURL + "?path=" + path.EncodeURL();
					return true;
				...
				if (code == 0 || code == 444 || (code >= 500 && code <= 599)) {
					...
					request.Url = CefMainWindow.CannotConnectURL;
					return true;
				}
